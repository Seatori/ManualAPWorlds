...
class RandomizeKeychains(Choice):
    """
    Add additional checks for the Keychains in Story Mode. Also adds the Rare Keychains to the pool as filler items.
    The Queen Sectonia Keychain obtained from collecting all Sun Stones is randomized, but has no equivalent check.
...
class GoalGames(DefaultOnToggle):
    """
    Add additional checks for doing each of the Goal Games across the stages.

    Technically, the checks are based on the Keychains that are awarded for winning, but since winning has no
    logical requirements, you're free to decide whether you need to get the Keychain or just do the Goal Game for
    yourself.
...
class KirbyFighters(Toggle):
    """
    Add additional checks for clearing Kirby Fighters with each of the available Copy Abilities.

    If Copy Abilities are randomized, they will have to be obtained before they can be used in Kirby Fighters.
    Otherwise, they will all be available immediately.
...
class ExtraStageKeys(DefaultOnToggle):
    ...
    If EX Stages are shuffled, the keys will unlock the Stage that was placed in the EX Stage's original position.
    """
...
class AbilityTestingRoom(Choice):
    ...
    'Randomized' means that the ability to access it will become a shuffled item. An equivalent check is not created.
...
class StageRando(Choice):
    ...
    'Extra Stages' means that the bonus stages that are normally unlocked by collecting all Sun Stones in a given
    level will be shuffled.
...
class BossRando(Choice):
    ...
    This allows for the goal to be placed in a fixed position that doesn't require any Grand Sun Stones, permitting the
    goal requirement to purely be a set amount of Sun Stones without intruding on the other bosses.
    """
    alias_false = 0
    alias_no = 0
    alias_off = 0
...
class LogicDifficulty(Choice):
    ...
    Easy only expects the player to use immediately accessible abilities, or if none are available, abilities that were
    presented to the player shortly before. It has a focus on using the 'intended' methods.
    ...
    Once the player gains access to an ability through any stage, boss, or the Copy Ability Testing Room, they can be
    expected to bring that ability to anywhere that it can be used.
    Damage boosting and using ability stars as projectiles can be also expected.
    Note that the player is never expected to bring Crash and Mike through mid-boss fights.
...
class SunStoneCount(Range):
    ...
    If there aren't enough checks for the number chosen, then Grand Sun Stones, EX Stage Keys, Copy Abilities,
    ...
    The value of this option may be modified by other factors, such as a limited number of checks being added.
...
class QueenSectoniaRequirement(NamedRange):
    ...
    The fight is always considered to be accessible immediately after defeating the prerequisite number of bosses, and
    the positions of the bosses has no impact on this.
...
class ExtraSunStones(Choice):
    ...
    This also makes it so that the excess are logically expected to be collected.
    ...
    'Removed' means that the excess will be completely removed from the pool and replaced by Keychains (or possibly
    Lose Ability Traps if enabled).
    This is similar to 'Filler', but designed to remove the confusion as to how many Sun Stones you're expected to have.
    """
    alias_yes = 0
    alias_keep = 0
    alias_prog = 0
